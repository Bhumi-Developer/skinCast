...
const Banners = () => {
  ...
  useEffect(() => {
  const completed = localStorage.getItem("formCompleted");
  const savedAnalysis = localStorage.getItem("analysis");

  if (!completed && !savedAnalysis) {
  ...
  return (
    <div className="w-full bg-linear-to-br from-primary-dull to-white px-4 py-2">
      <div className="max-w-7xl mx-auto h-full">
        <div className="relative rounded-3xl overflow-hidden shadow-2xl transition-all duration-300 h-[calc(100vh-100px)] md:h-[calc(100vh-120px)] lg:h-[calc(100vh-130px)] min-h-137.5 max-h-200">
          <div className="absolute inset-0 bg-linear-to-r from-primary/10 to-primary-middle/20"></div>
          ...
          >
            {banners.map((banner, idx) => (
              <div
                key={`${banner.id}-${idx}`}
                className="w-full shrink-0 relative z-10 flex flex-col lg:flex-row items-stretch h-full"
              >
                {/* Left side */}
                <div className="w-full lg:w-1/2 p-6 md:p-8 lg:p-10 xl:p-12 flex flex-col justify-center bg-white/80 backdrop-blur-sm lg:bg-transparent lg:backdrop-blur-0 h-full">
                  <span className="text-primary font-semibold text-xs md:text-sm uppercase tracking-wider mb-2 md:mb-3 flex items-center gap-2">
                  ...
                  <div className="flex flex-wrap gap-2 md:gap-3 mb-5 md:mb-6 lg:mb-7">
                    {banner.conditionBadges.map((condition, idx) => (
                      <span
                        key={idx}
                        className="bg-primary-dull text-primary-light px-3 md:px-4 py-1.5 md:py-2 rounded-full text-xs md:text-sm font-medium shadow-sm"
                      >
                        {condition}
                      </span>
                    ))}
                ...
                ( <button
                    ...
                    className="bg-primary hover:bg-primary-light text-white font-semibold py-2.5 md:py-3 px-6 md:px-8 rounded-xl transition duration-200 shadow-md hover:shadow-lg w-full sm:w-auto text-center flex items-center justify-center gap-2 text-sm md:text-base"
                  >
                    Create account and start analysis
                ...
                <div className="w-full lg:w-1/2 h-full relative">
                  ...
                  />
                  <div className="absolute inset-0 bg-linear-to-t from-primary/20 via-transparent to-transparent pointer-events-none"></div>
                </div>
              </div>
            ))}
          </div>
            ...
            {originalBanners.map((banner) => (
              <button
                ...
                className={`h-1.5 md:h-2 rounded-full transition-all duration-300 ${
                  current.id === banner.id ? 'w-6 md:w-8 bg-primary' : 'w-2 md:w-3 bg-white/60 hover:bg-white/80'
                }`}
