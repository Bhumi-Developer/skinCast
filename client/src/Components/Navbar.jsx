...
const Navbar = () => {
  ...
  const {logout, user} = useAuth();
   ...
   const { analysis, loading, error, fetchAnalysis } = useAnalysis();
 
  
    const userLocation = analysis?.location
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 md:h-20">
              ...
              >
                {({ isActive }) => (
                  <>
                    ...
                    <span className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 w-0 h-0.5 bg-primary rounded-full transition-all duration-300 group-hover:w-6 ${isActive ? 'hidden' : ''}`}></span>
                  </>
                )}
              </NavLink>
            ))}
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
           {weather ?  (<div className="hidden sm:flex items-center space-x-2 bg-linear-to-r from-primary-dull/40 to-white px-2 md:px-3 py-1.5 rounded-full border border-primary-dull shadow-sm">
              <span className="text-sm">
🌡️</span>
              <span className="text-xs text-gray-700 font-medium hidden md:inline">{weather?.temp ? Math.round(weather.temp) : "--"}°C • {weather.condition}</span>
                  ...
                  {isProfileOpen && (
                    <div className="absolute right-0 mt-2 w-56 sm:w-64 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden animate-fadeIn z-50">
                      <div className="p-3 sm:p-4 border-b border-gray-100">
                        <div className="flex items-center space-x-3">
                          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-linear-to-br from-primary to-primary-middle rounded-full flex items-center justify-center shrink-0">
                            <span className="text-white font-semibold text-xs sm:text-sm">{user?.name?.charAt(0)?.toUpperCase() || "U"}</span>
                          </div>
                      ...
                      <div className="py-2">
                        ...
                        <NavLink to="/profile/settings" onClick={handleLinkClick} className="flex items-center px-3 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 hover:bg-primary-dull/30 hover:text-primary transition-colors duration-200">
                          <svg className="w-4 h-4 mr-3 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button
              ...
              className="md:hidden p-1.5 sm:p-2 rounded-lg text-gray-600 hover:text-primary hover:bg-primary-dull/50 transition-all duration-300"
            >
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `px-4 py-3 rounded-lg text-base font-medium transition-all duration-300 ${
                      isActive
                        ? 'text-primary bg-primary-dull/50'
                        : 'text-gray-600 hover:text-primary hover:bg-primary-dull/30'
                    }`
                  }
                >
                  {link.name}
              ...
              <div className="pt-3 px-4 border-t border-gray-100 mt-2 space-y-3">
                {!user ? (
                  <button 
                    ...
                    className="w-full bg-linear-to-r from-primary to-[#C48476] text-white font-semibold py-3 rounded-xl text-center transition-all duration-300 hover:shadow-md"
                  >
                    Sign In
                  </button>
                ) : (
                  <>
                    {/* <NavLink
                      to="/get-started"
                      onClick={handleLinkClick}
                      className="w-full bg-linear-to-r from-primary to-[#C48476] text-white font-semibold py-3 rounded-xl text-center block transition-all duration-300 hover:shadow-md"
                    >
                      Get Started
                    </NavLink> */}
                    <button 
