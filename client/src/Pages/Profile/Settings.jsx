...
//                           className="w-full text-left px-5 py-2 text-primary hover:bg-primary-dull/20 transition-colors text14 flex items-center gap-2 border-t border-primary-dull/20 mt-1"
//                         >
...
const Settings = () => {
  ...
  const categoryOptions = [
    { value: 'skin care', label: 'Skin Care' },   // ✅ match Form values
  ...
  const handleSave = () => {
    ...
    const newRecord = {
      ...
      budget: profile.budget,
      concerns: profile.concerns,
      productGoals: profile.productGoals,   // ✅ fixed
      location: profile.location,
      gender: profile.gender,
      category: profile.category,
    };
                  ...
                  >
                    <span className="text-gray-700">
                      {profile.productGoals.length > 0
                        ? `${profile.productGoals.length} selected`
                        : 'Select goals'}
                    </span>
                    <span className="text-primary text-xl">{showGoals ? '▲' : '▼'}</span>
                  </div>
